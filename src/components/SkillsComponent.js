...
const SkillsComponent = () => {
  return <section id='skills' className='py-10'>
    <div>
        ...
        <div className="hidden md:grid grid-cols-3 gap-10 place-items-center">
          {
            iconSkills.map((item, id) => (
              <div className='flex flex-col items-center gap-5'>
                <Icon key={id} className={`saturate-0 ${item.name === 'Bootstrap' && 'hover:contrast-100 contrast-0'} hover:saturate-100 cursor-pointer duration-500 hover:scale-110`} icon={item.icon} width={item.width} height={item.height} />
        ...
        <div className="md:hidden grid grid-cols-3 place-items-center">
          {
            iconSkills.map((item, id) => (
              <div className='py-5 flex flex-col items-center gap-5'>
                <Icon key={id} className={`saturate-0 ${item.name === 'Bootstrap' && 'hover:contrast-100 contrast-0'} hover:saturate-100 cursor-pointer duration-500 hover:scale-110`} icon={item.icon} width='60' height='60' />
                <span className='md:text-lg text-sm font-medium dark:text-slate-400 text-slate-500'>{item.name}</span>
              </div>
            ))
          }
